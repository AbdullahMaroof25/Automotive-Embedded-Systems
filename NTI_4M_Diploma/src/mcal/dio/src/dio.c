...
{
	/*	show me your code */
	dio_level_t status = STD_LOW;
	
	switch( port )
	{
		case DIO_PORTA:
		status = CHECK_BIT(DIO_PINA_REG,channel);
		break;
		
		case DIO_PORTB:
		status = CHECK_BIT(DIO_PINB_REG,channel);
		break;
		
		case DIO_PORTC:
		status = CHECK_BIT(DIO_PINC_REG,channel);
		break;
		
		case DIO_PORTD:
		status = CHECK_BIT(DIO_PIND_REG,channel);
		break;
